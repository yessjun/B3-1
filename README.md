...
보너스 과제로 도메인 `codyssey.run`을 연결하고 HTTPS도 적용했습니다. 실습에 사용한 리소스는 모두 삭제했으므로 지금 접속되는 주소는 없습니다. 각 단계의 명령과 화면은 아래에 그대로 남겨두었습니다.
...
보너스 절에서 도메인과 HTTPS를 붙인 환경은 같은 구성을 한 번 더 만든 것인데, 이것도 인스턴스 종료, VPC 삭제, 키 페어 삭제 순으로 같이 지웠습니다. 인스턴스를 지우면 퍼블릭 IP가 회수되므로 도메인의 A 레코드도 같이 정리해야 합니다. 남겨두면 나중에 그 주소를 받은 다른 사용자를 가리키게 됩니다.
